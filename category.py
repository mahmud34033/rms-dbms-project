...
class categoryClass:
    def __init__(self, root):
        ...
        #=======Title===============
        lbl_title = Label(self.root, text="Manage Items Category", font=("goudy old style", 30), bg="#184a45", fg="white", bd=3, relief=RIDGE).pack(side=TOP, fill=X, padx=10, pady=20)
        ...
        #=======Buttons===============
        btn_add = Button(self.root, text="ADD",command=self.add, font=("goudy old style", 15), bg="#04CA50", fg="white", cursor="hand2")
        ...
        self.im1 = Image.open("images/food_items.jpg")
        self.im1 = self.im1.resize((615, 250), Image.Resampling.LANCZOS)
        ...
        try:
            if self.var_name.get() =="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success", "Category added successful",parent=self.root)
                    self.show()
        except Exception as ex: 
            messagebox.showerror("Error",f"Error due to (add) : {str(ex)}",parent=self.root)
