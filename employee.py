...
class employeeClass:
    def __init__(self, root):
        ...
        #===options====
        cmd_search=ttk.Combobox(SearchFrame,textvariable=self.var_searchby,values=("Select","Email","Name","Contact","Role"),state='readonly',justify=CENTER, font=("goudy old style", 15))
        ...
        cmd_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("Select","Male","Female"),state='readonly',justify=CENTER, font=("goudy old style", 15))
        ...
        lbl_role=Label(self.root,text="Role",font=("goudy old style",15),bg="white").place(x=750,y=230)
        ...
        cmd_role=ttk.Combobox(self.root,textvariable=self.var_role,values=("Select","Admin","Manager","Cashier","Waitress","Chef","Cleaner","Water Boy"),state='readonly',justify=CENTER, font=("goudy old style", 15))
        cmd_role.place(x=850,y=230,width=180)
        cmd_role.current(0)
        ...
        #===buttons====
        btn_add = Button(self.root, text="Save", command=self.add, font=("goudy old style", 13), bg="#2196f3", fg="white", cursor="hand2").place(x=500, y=305, width=110, height=28)
        btn_update = Button(self.root, text="Update", command=self.update, font=("goudy old style", 13), bg="#4caf50", fg="white", cursor="hand2").place(x=620, y=305, width=110, height=28)
        btn_delete = Button(self.root, text="Delete", command=self.delete, font=("goudy old style", 13), bg="#f44336", fg="white", cursor="hand2").place(x=740, y=305, width=110, height=28)
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=("goudy old style", 13), bg="#607d8b", fg="white", cursor="hand2").place(x=860, y=305, width=110, height=28)
        ...
        self.EmployeeTable=ttk.Treeview(emp_frame,columns=("eid","name","email","gender","contact","dob","doj","pass","role","address","salary"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.EmployeeTable.column("email",width=130)
        self.EmployeeTable.column("gender",width=100)
        self.EmployeeTable.column("contact",width=100)
        self.EmployeeTable.column("dob",width=100)
        self.EmployeeTable.column("doj",width=100)
        self.EmployeeTable.column("pass",width=100)
        self.EmployeeTable.column("role",width=100)
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row!=None:
                    ...
                    cur.execute("Insert into employee (eid,name,email,gender,contact,dob,doj,pass,role,address,salary) values(?,?,?,?,?,?,?,?,?,?,?)",(
                        self.var_emp_id.get(),
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("Update employee set name=?,email=?,gender=?,contact=?,dob=?,doj=?,pass=?,role=?,address=?,salary=? where eid=?",(
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
                        self.var_contact.get(),
